...
//							Log.d("rx_fix", "start");
							...
							bytes = mmInStream.read(buffer); // 读入数据
...
//									Log.d("rx_fix", "stop");
					...
					{
                        //in this block we will do some thing,and i think if this thread is block ,then we will 
                        //lost some data
...
//						if(str!="")
						{
	                     mHandler.obtainMessage(BluetoothSerial.MESSAGE_READ, bytes, -1, str)
	                            .sendToTarget();
	                     str="";
	                    ok=false;
						}
